...
const buildStrategy = (): Strategy => {
  if (data.race) {
    ...
    let networkTimeoutSeconds = data.networkTimeoutSeconds
          ...
          if (!manifestURLs.includes(request.url)) {
            cache.delete(request).then((deleted) => {
              if (data.debug) {
                if (deleted)
                  console.log(
                    `Pre-cached data removed: ${request.url || request}`
                  )
                else
                  console.log(`No pre-cache found: ${request.url || request}`)
              }
            })
          }
        })
